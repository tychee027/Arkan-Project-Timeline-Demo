...
// ── Bulk insert (for large imports, e.g. Excel) ───────
// Chunks large arrays into multiple requests to avoid payload/timeout limits,
// and returns all inserted rows (with their generated ids) concatenated together.
async function insertInChunks(table, rows, chunkSize = 500) {
  ...
  for (let i = 0; i < rows.length; i += chunkSize) {
    ...
    if (error) { trackError(table + ' bulk insert error', error); throw error }
...
export async function addProjectsBulk(rows) {
  return insertInChunks('projects', rows, 500)
}

export async function addTasksBulk(rows) {
  return insertInChunks('tasks', rows, 500)
}

async function deleteInChunks(table, column, ids, chunkSize = 500) {
  for (let i = 0; i < ids.length; i += chunkSize) {
    const chunk = ids.slice(i, i + chunkSize)
    const { error } = await supabase.from(table).delete().in(column, chunk)
    if (error) { trackError(table + ' bulk delete error', error); throw error }
  }
}

// Deletes multiple projects and all of their tasks in a few chunked requests
// instead of one request per project.
export async function deleteProjectsBulk(ids) {
  await deleteInChunks('tasks', 'pid', ids, 500)
  await deleteInChunks('projects', 'id', ids, 500)
  lastError = null
}
